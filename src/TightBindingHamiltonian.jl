...
For each real space coordinate of type `Vector{I}`,
hopping parameters are stored as `Matrix{F}`.
"""
...
Evaluate `H` at a momentum `k` and store it in `M`

If no container is given, it is stored in `H.Hk`.
